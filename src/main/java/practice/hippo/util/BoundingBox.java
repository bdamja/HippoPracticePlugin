...
public class BoundingBox implements Cloneable, ConfigurationSerializable {
    ...
    public boolean containsXandZInclusive(double x, double z) {
        return x >= this.minX && x <= this.maxX
                && z >= this.minZ && z <= this.maxZ;
    }
    ...
    public boolean containsXandZInclusive(@NotNull Vector position) {
        Validate.notNull(position, "Position is null!");
        return this.containsXandZInclusive(position.getX(), position.getZ());
    }
